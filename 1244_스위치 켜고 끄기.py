...
for i in range(num_stu):
    if (sex_num[i][0] == 1):                    # 남자이면
        for j in range(1, num_sw):               #스위치 개수 안에서 돌림
            if j % (sex_num[i][1]) == 0:        #뽑은 숫자가 스위치 번호의 배수일때
                pre_sw[j] = 1 - pre_sw[j]       #1이면 0을, 0이면 1을 출력
            else:                               # 배수가 아닐 때
                continue
        i += 1                                       #다음 학생 부르기 위해.~

    if (sex_num[i][0] == 2):                            #여자이면
                                            # pre_sw[(sex_num[i][1])+1]  :   자기가 받은 수와 같은 번호가 붙은 스위치
        for k in range(1, min((num_sw - pre_sw[(sex_num[i][1])+1]), (pre_sw[(sex_num[i][1])+1]-1) )): #최소 구간 지정
            count = 0
            ...
            #else:
            #    break
            for a in range(pre_sw[(sex_num[i][1]) + 1 - k], pre_sw[(sex_num[i][1]) + 1 + k]):  # 해당 구간에 해당하는 스위치는
